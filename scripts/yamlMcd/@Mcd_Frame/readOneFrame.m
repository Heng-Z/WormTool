...
while ~isEndOfFrame(tline)
    if isField(tline)
        ...
        switch fname
            ...
            case 'LaserPower'
                [mcdf.FirstLaser mcdf.SecondLaser tline]=getLaserPower(fid,tline);
            otherwise
...
function[First Second tline]=getLaserPower(fid,tline)
...
if ~isFieldWithValue(tline)
    ...
    tline=fgets(fid);
    fname= getField(tline);
    while strcmp(fname,'FirstLaser') || strcmp(fname,'SecondLaser')
        switch fname
            case 'FirstLaser'
                [First tline]=getVal(fid,tline);
                fname= getField(tline);
            case 'SecondLaser'
                [Second tline]=getVal(fid, tline);
                fname= getField(tline);
